...
export async function GET() {
  try {
    ...
    const result = await sql<LocationRow>`SELECT * FROM medications;`;
    ...
    console.error('Error fetching mocations:', error);
    return NextResponse.json({ error: 'Failed to fetch mocations' }, { status: 500 });
  }
}