import type { DecimalType } from "./DecimalType.js";
import type { MarkingType } from "./MarkingType.js";
import type { PrimitiveType } from "./PrimitiveType.js";
import type { StructType } from "./StructType.js";
import type { VectorType } from "./VectorType.js";
export interface ValueType_array {
    type: "array";
    array: ValueType;
}
export interface ValueType_decimal {
    type: "decimal";
    decimal: DecimalType;
}
export interface ValueType_marking {
    type: "marking";
    marking: MarkingType;
}
export interface ValueType_primitive {
    type: "primitive";
    primitive: PrimitiveType;
}
export interface ValueType_struct {
    type: "struct";
    struct: StructType;
}
export interface ValueType_vector {
    type: "vector";
    vector: VectorType;
}
export type ValueType = ValueType_array | ValueType_decimal | ValueType_marking | ValueType_primitive | ValueType_struct | ValueType_vector;
//# sourceMappingURL=ValueType.d.ts.map